...
export class DockerBuilder implements IBuilder {
    ...
    async build(documentUri: vscode.Uri): Promise<BuildResult> {
        ...
        try {
            ...
            const volumeMounts = [
                `-v "${docDir}:/workspace"`,
                this.config.getDockerEnableCache() ? `-v ${this.volumeName}:/usr/local/texlive` : ''
            ].filter(Boolean).join(' ');
    ...
    private async ensureCacheVolume(): Promise<void> {
        try {
            ...
            const { stdout } = await execAsync(`docker volume ls -q -f name=${this.volumeName}`);
